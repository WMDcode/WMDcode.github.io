...
nyc_burgers |>
  ...
  summarise(
    ...
    .groups = "drop") |>
  mutate(
    name_clean = fct_reorder(name_clean, score)) |>
  plot_ly(
    y = ~score, 
    color = ~name_clean, 
    type = "box", 
  ...
  layout(
    showlegend = FALSE,
    xaxis = list(title = "Burger Restaurant"),
    yaxis = list(title = "Sanitation Score"))
```
...
### Number of Burger Restaurants by Manhattan Neighborhood and Sanitation Grade
...
left_join(nyc_burgers, zipcodes_df, by = "zipcode") |>
  ...
  pivot_longer(
    ...
    values_to = "count") |>
  mutate(
    neighborhood = case_match(
      neighborhood,
      NA_character_ ~ "Other",
      .default = neighborhood),
    neighborhood = fct_reorder(neighborhood, total, .desc = TRUE)) |>
  plot_ly(
    x = ~neighborhood,
    y = ~count,
    color = ~grade,
    colors = "viridis") |>
  layout(
    xaxis = list(title = "Manhattan Neighborhood"),
    yaxis = list(title = "Number of Burger Restaurants"))
```
